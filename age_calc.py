...
class AgeCalculator(Frame):
    def __init__(self,root):
        ...
        self.main_frame = Frame(borderwidth=2,relief='ridge',width=300,height=245,bg='#fff')
        self.main_frame.place(x=20,y=60)
        Label(self.main_frame,text='Date of Birth:',bg='#fff',fg='teal',font='montserrat 12').place(x=10,y=20)
        self.dob_date = DateEntry(self.main_frame,width=12, background='teal',
                font='montserrat 12',foreground='white',borderwidth=5)
        self.dob_date.place(x=10,y=50)

        Label(self.main_frame,text='Target Date:',bg='#fff',fg='teal',font='montserrat 12').place(x=10,y=90)
        self.target_date = DateEntry(self.main_frame,width=12, background='teal',
                font='montserrat 12',foreground='white',borderwidth=5)
        self.target_date.place(x=10,y=120)

        Button(self.main_frame,text='Calculate',fg='#fff',bg='teal',width=8,bd=0,
            font='montserrat 14',command=self.calculate_it).place(x=10,y=160)

        self.result_frame = Frame(borderwidth=2,relief='ridge',width=300,height=250,bg='#fff')
        self.result_frame.place(x=350,y=60)
        # Label(self.main_frame,text='Age Calculator').place(x=10, y=30)
